...
import com.javabasesetupmvvm.data.model.others.QuestionCardData;
...
public class MainViewModel extends BaseViewModel<MainNavigator> {
    ...
    public void loadQuestionCards() {
        getCompositeDisposable().add(getDataManager()
                .getQuestionCardData()
                .doOnNext(list -> Timber.d("loadQuestionCards: " + list.size()))
                .subscribeOn(getSchedulerProvider().io())
                .observeOn(getSchedulerProvider().ui())
                .subscribe(questionList -> {
                    if (questionList != null) {
                        Timber.d("loadQuestionCards: " + questionList.size());
                        action = ACTION_ADD_ALL;
                        questionCardData.setValue(questionList);
                    }
                }, throwable -> {
                    Timber.d("loadQuestionCards: " + throwable);
                }));
    }
